...
async function processFileDirectlyFromCSV(fileContent, lines) {
  ...
  // Extremely small batches for very large files to prevent blocking and allow UI interaction
  let batchSize;
  if (totalRows < 5000) {
    ...
    batchSize = 20;    // Large files: extremely small batches to prevent blocking
  } else {
    batchSize = 10;    // Very large files: minimal batches for maximum responsiveness
  }
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    // Process batch with periodic yields to prevent blocking
    // For very large files, yield every 10 rows within a batch to keep UI responsive
    const shouldYieldInBatch = totalRows > 10000;
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      // Yield every 10 rows within batch for very large files to keep UI responsive
      // Use multiple yields to truly release the thread and allow browser to render
      if (shouldYieldInBatch && (i - startIndex) % 10 === 0 && (i - startIndex) > 0) {
        // Triple yield: nextTick, setImmediate, then setTimeout to allow browser to render
        await new Promise(resolve => process.nextTick(resolve));
        await new Promise(resolve => setImmediate(resolve));
        await new Promise(resolve => setTimeout(resolve, 20)); // Balanced delay (20ms) for browser to render and allow scrolling
      }
    }
    
    allLeads.push(...batchLeads);
    
    // Progress feedback every 10 batches or on last batch (less frequent for large files)
    ...
    // Yield after EVERY batch for files > 10000 rows to prevent blocking
    // Use multiple yields with balanced delays to truly release the thread
    // This allows the browser to process events, render updates, and allow scrolling
    if (totalRows > 10000) {
      // Triple yield: nextTick, setImmediate, then setTimeout with balanced delay
      // This ensures the event loop is fully released and browser can render and scroll
      await new Promise(resolve => process.nextTick(resolve));
      await new Promise(resolve => setImmediate(resolve));
      await new Promise(resolve => setTimeout(resolve, 30)); // Balanced delay (30ms) for browser to render and allow scrolling
    } else if (totalRows > 5000) {
      // Yield every batch for medium-large files
      await new Promise(resolve => process.nextTick(resolve));
      await new Promise(resolve => setImmediate(resolve));
      await new Promise(resolve => setTimeout(resolve, 15));
    } else if (batchIndex % 5 === 0 && batchIndex > 0) {
      // Yield every 5 batches for smaller files
