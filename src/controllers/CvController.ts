...
class CVController {
  // Initialize OpenAI instance for each request
  getOpenAIInstance() {
  ...
  extractBasicInfo = async (req, res) => {
    try {
      ...
      return res.status(500).json({ 
        error: 'Failed to extract basic information from CV',
        details: error.message 
      });
    }
  }

  analyzeExperience = async (req, res) => {
    try {
      const openai = this.getOpenAIInstance();
      const { contentToProcess } = req.body;
      ...
      return res.status(500).json({ error: 'Failed to analyze experience', details: error.message });
    }
  }
  ...
  analyzeSkills = async (req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'Failed to analyze skills', details: error.message });
    }
  }

  analyzeAchievements = async (req, res) => {
    try {
      const openai = this.getOpenAIInstance();
      const { contentToProcess } = req.body;
      ...
      return res.status(500).json({ error: 'Failed to analyze achievements', details: error.message });
    }
  }

  analyzeAvailability = async (req, res) => {
    try {
      const openai = this.getOpenAIInstance();
      const { contentToProcess } = req.body;
      ...
      return res.status(500).json({ error: 'Failed to analyze availability', details: error.message });
    }
  }
